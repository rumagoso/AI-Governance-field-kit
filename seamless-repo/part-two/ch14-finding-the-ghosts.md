...
**Run date:** 1 March 2027, triggered directly by the bank client's annual security questionnaire, which asked Ondas to list every AI tool with access to client data
**Method used:** OAuth/SSO grant review, the only one of the four methods a twenty-four person company could run without dedicated tooling
**Finding:** Four "Sign in with Google Workspace" grants to AI writing and image tools, none on any approved list, because Ondas didn't have one until this questionnaire arrived; granted by four different people between mid-2025 and early 2027
**Action:** Two tools folded into the tiered AUP from Chapter 4; one, used only for internal notes, blocked outright; one turned out to be the video-generation tool already covered properly in Chapter 12

The point isn't that ghosts got found. It's that a client's own due diligence did the discovery work Ondas had never done for itself, which is a more common way this actually happens than most companies like to admit.
...
ENISA's 2026 frontier-cybersecurity report cites the Vercel/Context.ai breach that opens this chapter's companion essay as a live example of exactly this pattern, independent confirmation that the incident named there is not an edge case but a shape regulators are now tracking directly.
